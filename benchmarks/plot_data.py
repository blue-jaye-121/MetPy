...
# Subsample step size (e.g., every 5th point → 10 steps in each dimension)
step = 1

# Get sampled index arrays
pressure_sub = np.arange(0, ds.pressure.size, step)
lat_sub = np.arange(0, ds.lat.size, step)
lon_sub = np.arange(0, ds.lon.size, step)

# Build subsampled 3D grid
lon_vals = ds.lon.values[lon_sub]
lat_vals = ds.lat.values[lat_sub]
pressure_vals = ds.pressure.values[pressure_sub]
height_vals = ds.height.values[np.ix_(pressure_sub, lat_sub, lon_sub)]

lon3d, lat3d, pressure3d = np.meshgrid(lon_vals, lat_vals, pressure_vals, indexing='ij')
lon3d = np.transpose(lon3d, (2, 1, 0))       # (pressure, lat, lon)
lat3d = np.transpose(lat3d, (2, 1, 0))
pressure3d = np.transpose(pressure3d, (2, 1, 0))

# Coordinates to plot (flattened)
x = lon3d.flatten()
y = lat3d.flatten()
z = height_vals.flatten()

# Time index to plot
time_index = 0

# Variables to plot (same list as before)
variables = {
    ...
    'W Wind': ds.wwind,
    'Theta': ds.theta,
    'Mixing Ratio': ds.mixing_ratio,
    'Vapor Pressure': ds.vapor_pressure,
    'Dewpoint': ds.dewpoint,
    'Relative Humidity': ds.relative_humidity,
    'Wind Speed': ds.windspeed,
    'Wind Direction': ds.winddir,
    'Sigma': ds.sigma,
    'Geopotential': ds.geopotential,
    'Specific Humidity': ds.specific_humidity,
    'Wet Bulb Temp': ds.wet_bulb_temperature,
    'Omega': ds.omega
}
...
for idx, (var_name, var_data) in enumerate(variables.items()):
    ax = fig.add_subplot(rows, cols, idx + 1, projection='3d')
        ...
        # Inside the loop, after creating `ax`
    pos = ax.get_position()
    ax.set_position([pos.x0, pos.y0, pos.width * 0.8, pos.height])  # Shrink width by 20%
    
    # Extract variable data and subsample
    var_sub = var_data.isel(
        time=time_index,
        pressure=pressure_sub,
        lat=lat_sub,
        lon=lon_sub
    ).values.flatten()
    
    # Plot 3D scatter
    sc = ax.scatter(x, y, z, c=var_sub, cmap='coolwarm', s=3, alpha=0.7)
    ax.set_xticks([])
    ax.set_yticks([])
    ax.set_zticks([])
    ...
    # Colorbar
    cbar = fig.colorbar(sc, ax=ax, shrink=0.6, pad=0.2)
    cbar.ax.tick_params(labelsize=16)
    units = var_data.attrs.get('units', '')
    cbar.set_label(units, fontsize=16)

    # Optional: make cube look good
    ax.set_box_aspect([1, 1, 0.6])

# Title and layout
fig.suptitle(f'Benchmarking Dataset at time = {str(ds.time.values[time_index])[:10]}', fontsize=36)
plt.tight_layout()
